...
# # Create a complete sequence of years from 1989 to 2020
# all_years <- data.frame(year = 1989:2020)
# 
...
# calculate_cagr <- function(gdp_start, gdp_end, years) {
#   if (gdp_start == 0 || gdp_end == 0) return(NA)  # Avoid invalid calculations
#   return((gdp_end / gdp_start)^(1 / years) - 1)
# }
# 
# # Create an empty data frame for extrapolated GDP values
# extrapolated_gdp_grid <- data.frame()
# 
# # Iterate over each grid and extrapolate GDP values
...
# 
#   # Calculate CAGR
#   cagr <- calculate_cagr(gdp_start, gdp_end, 2025 - 1989)
# 
#   # Extrapolate GDP values for all years
#   temp_df <- data.frame(
#     grid_id = grid_id,
#     year = 1989:2025,
#     gdp_value = gdp_start * (1 + cagr)^(1989:2025 - 1989)
#   )
...
# 
# # Verify the change
# # Summarize the extrapolated GDP grid
# summary(extrapolated_gdp_grid)
# 
# # write_rds(extrapolated_gdp_grid, "./Data/data-processed/extrapolated_gdp_grid.rds")
# 
# extrapolated_gdp_grid_until_2020 <- extrapolated_gdp_grid %>%
#   filter(year <= 2020)
# # write_rds(extrapolated_gdp_grid_until_2020, "./Data/data-processed/extrapolated_gdp_grid_until_2020.rds")
# 
# 
...
The datas are 
- ethnic_geo_ref_2010_eth (geo-referencing of Ethnic groups) 8969 obs - it is not filtered to country yet, this seems to be unusable
  the ethnic group is coded based on one of the six settlement pattern 
- political_ethnic_country_epr_eth (ethnic group with relevant political power in country) 280 obs, maybe using the original data is a good idea (geo_ethnic_power_geoepr_eth)
- geo_ethnic_power_geoepr_country_eth (76 obs) (with polygon reference)
- conflict_ethnic_acd2epr_country_eth (Conflict btw Ethnic) - only 33 obs -> not really interesting
- Ethno-Linguistic, religious and 

The second and the third data go along together

Faster example of visualization: https://growup.ethz.ch/pfe/country
The data now is political_ethnic_country_epr_eth_filtered

```{r}
# # Filter out rows 1, 2, and 21
# filtered_data <- political_ethnic_country_epr_eth_filtered[-c(16, 20, 21), ]
# 
# # Create the plot
# plot_country_single_ethnic <- ggplot() +   
#   geom_sf(data = grid_intersect_bws_test, color = "black", size = 0.2, fill = NA) +
#   geom_sf(data = filtered_data, color = "red", size = 0.5, fill = NA) + # Highlight in red or any desired color
#   geom_sf_text(data = filtered_data, aes(label = group), size = 3, color = "blue") + # Add labels
#   theme_minimal() +
#   labs(title = "Map of country Highlighting Specific Ethnic Power Rows (Excluding Rows 1, 2, 21)",
#        x = "Longitude",
#        y = "Latitude")
# 
# # Display the plot
# plot_country_single_ethnic
# 
